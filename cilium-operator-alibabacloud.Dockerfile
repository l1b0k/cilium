...
ARG LOCKDEBUG
ARG RACE
RUN make NOSTRIP=$NOSTRIP LOCKDEBUG=$LOCKDEBUG RACE=$RACE cilium-operator-alibabacloud
...
LABEL cilium-sha=${CILIUM_SHA}
LABEL maintainer="maintainer@cilium.io"
COPY --from=builder /go/src/github.com/cilium/cilium/operator/cilium-operator-alibabacloud /usr/bin/cilium-operator-alibabacloud
...
WORKDIR /
CMD ["/usr/bin/cilium-operator-alibabacloud"]
