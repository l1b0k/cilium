...
var log = logging.DefaultLogger.WithField(logfields.LogSubsys, "alibaba-cloud-openapi")
...
func (c *Client) GetVSwitches(ctx context.Context) (ipamTypes.SubnetMap, error) {
	...
	for i := 0; i <= (len(vsws)-1)/MaxListByTagSize; i++ {
		...
		tail := min((i+1)*MaxListByTagSize, len(vsws))
		...
		for _, tagRes := range resp.TagResources.TagResource {
			...
			if !ok {
				log.Errorf("can not found vSwitch id %s", tagRes.ResourceId)
				continue
			}
...
func (c *Client) CreateNetworkInterface(ctx context.Context, toAllocate int, vSwitchID string, groups []string, tags map[string]string) (string, *eniTypes.ENI, error) {
