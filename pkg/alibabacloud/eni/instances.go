...
type AlibabaCloudAPI interface {
	...
	CreateNetworkInterface(ctx context.Context, toAllocate int, vSwitchID string, groups []string, tags map[string]string) (string, *eniTypes.ENI, error)
...
// AllocIndexForENI will alloc an monotonically increased index for each ENI on this instance.
// The index generated the first time this ENI is created, and stored in ENI.Tags.
func (m *InstancesManager) AllocIndexForENI(instanceID string) (int, error) {
	// alloc index for each created ENI
	used := make([]bool, maxENIPerNode)
	err := m.instances.ForeachInterface(instanceID, func(instanceID, interfaceID string, iface ipamTypes.InterfaceRevision) error {
		cachedENI, ok := iface.Resource.(*eniTypes.ENI)
		if !ok {
			return fmt.Errorf("ENI type mismatch, got %T", iface.Resource)
		}
		index := GetENIIndexFromTags(cachedENI.Tags)
		if index > maxENIPerNode || index < 0 {
			return fmt.Errorf("ENI index(%d) is out of range", index)
		}
		used[index] = true
		return nil
	})
	if err != nil {
		log.WithError(err).Warning("Unable to retrieve index for ENI")
		return 0, err
	}
