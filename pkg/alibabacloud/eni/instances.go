...
type AlibabaCloudAPI interface {
	...
	CreateNetworkInterface(ctx context.Context, toAllocate int, vSwitchID string, groups []string) (string, *eniTypes.ENI, error)
...
func (m *InstancesManager) GetVSwitches(ctx context.Context) ipamTypes.SubnetMap {
	m.mutex.RLock()
...
// FindOneVSwitch return vSwitch with least addresses matching vpc, az and tags
func (m *InstancesManager) FindOneVSwitch(vpc, az string, toAllocate int, required ipamTypes.Tags) *ipamTypes.Subnet {
	...
	for _, vSwitch := range m.GetVSwitches(context.TODO()) {
