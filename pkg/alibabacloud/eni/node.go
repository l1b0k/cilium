...
func (n *Node) CreateInterface(ctx context.Context, allocation *ipam.AllocationAction, scopedLog *logrus.Entry) (
	...
	toAllocate = math.IntMin(maxENIIPCreate, toAllocate) // in first alloc no more than 9
	...
	index, err := n.manager.AllocIndexForENI(n.node.InstanceID())
	if err != nil {
		n.manager.mutex.Unlock()
		scopedLog.WithField("instanceID", n.node.InstanceID()).Error(err)
		return 0, "", err
	}
	eniID, eni, err := n.manager.api.CreateNetworkInterface(ctx, toAllocate, bestSubnet.ID, securityGroupIDs, map[string]string{eniIndexTagKey: strconv.Itoa(index)})
	n.manager.mutex.Unlock()
	if err != nil {
